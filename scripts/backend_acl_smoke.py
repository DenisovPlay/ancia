...
# И не пытаемся инициализировать MLX runtime (в CI/песочнице это может завершать процесс).
...
def main() -> int:
  failed = False
  mlx_runtime_disabled = os.getenv("ANCIA_DISABLE_MLX_RUNTIME", "").strip() == "1"
  ...
  original_env = {
    ...
    "ANCIA_DISABLE_MLX_RUNTIME": os.environ.get("ANCIA_DISABLE_MLX_RUNTIME"),
  ...
  try:
    ...
    with create_app_client(app) as client:
      ...
      if uncached_model_id and not mlx_runtime_disabled:
        for _ in range(20):
          ...
          if loaded_model_id == uncached_model_id:
            should_check_chat_acl_bypass = True
            break
          if startup_model_id == uncached_model_id and startup_status in {"loading", "ready"}:
            should_check_chat_acl_bypass = True
            break
      ...
      elif mlx_runtime_disabled:
        print("[OK] skipped limited chat ACL-bypass check (MLX runtime disabled for smoke)")
      else:
